package com.devsu.accountmovement.exception;
...
import static com.devsu.accountmovement.utils.Constants.HTTP_MESSAGE1;
import static com.devsu.accountmovement.utils.Constants.HTTP_MESSAGE2;
...
public class ApiExceptionHandler {
    ...
    @ResponseBody
    public ResponseEntity<ReplyMessage> apiConnectionException(ConnectException exception) {
        List<String> messages = new ArrayList<>();
        messages.add(API_ACONNECT_ERROR);
        messages.add(exception.getMessage());
        messages.add(exception.getLocalizedMessage());
        messages.add(exception.getCause().toString());
        messages.add(exception.getStackTrace().toString());
        replyMessage.setHttpStatus(HttpStatus.BAD_REQUEST);
        replyMessage.setError(true);
        replyMessage.setMessage(messages);
        replyMessage.setObject(null);
